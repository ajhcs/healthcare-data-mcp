...
def test_all_six_rosters_and_bed_coverage_are_complete() -> None:
    ...
    assert {
        item.value for item in observations.values() if item.measure_id == "hospital_roster_disposition"
...
def test_handoff_lineage_resolves_and_schema_validates() -> None:
    ...
    assert all(observation.receipt_refs and set(observation.receipt_refs) <= receipts for observation in bundle.observations)
    ...
    assert all(source.receipt.artifact.connector_version == "scale-roster-bed-connector.v1" for source in bundle.sources)
...
def test_official_rights_remain_review_required_and_government_sources_are_public_domain() -> None:
    ...
    assert by_id["cms-pos-q4-2025"].rights_classification == "public_domain"
    assert by_id["pa-hospital-reports-index"].rights_classification == "public_domain"
