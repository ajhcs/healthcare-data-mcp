...
def get_client() -> httpx.AsyncClient:
    ...
    if _client is None or _client.is_closed:
        _client = httpx.AsyncClient(
            ...
            # Default timeouts; callers can override per-request via the
            # ``timeout`` kwarg on resilient_request.
    ...
    *,
    ...
    for attempt in range(1 + max_retries):
        try:
            ...
            # Retryable status code — compute delay
            ...
            # Final attempt — raise
            resp.raise_for_status()
            return resp  # unreachable, raise_for_status always throws for 4xx/5xx
...
def _parse_retry_after(resp: httpx.Response) -> float | None:
    ...
    try:
        ...
        pass
    # RFC 7231 HTTP-date (e.g., "Fri, 31 Dec 1999 23:59:59 GMT")
    try:
...
class _OSRMRateLimiter:
    ...
    Implemented with an ``asyncio.Lock`` rather than a ``Semaphore`` so that
    we can enforce a *time gap* between consecutive requests, not just
    concurrency.
    """
