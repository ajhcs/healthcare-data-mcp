...
async def _download_csv(url: str, cache_name: str) -> Path:
    """Download a CSV from CMS and cache it locally, respecting TTL."""
...
async def load_hospital_info() -> pd.DataFrame:
    ...
    async with _df_lock:
        if _hospital_info_df is not None:
            cache_path = CACHE_DIR / "hospital_general_info.csv"
            if is_cache_valid(cache_path, max_age_days=_BULK_TTL_DAYS):
                return _hospital_info_df
            _hospital_info_df = None
...
async def load_cost_report() -> pd.DataFrame:
    ...
    async with _df_lock:
        if _cost_report_df is not None:
            cache_path = CACHE_DIR / "hospital_cost_report.csv"
            if is_cache_valid(cache_path, max_age_days=_BULK_TTL_DAYS):
                return _cost_report_df
            _cost_report_df = None
