...
def main() -> None:
    ...
    for run_name, (arm, repetition) in expected.items():
        path = root / "runs" / f"{run_name}.json"
        ...
        assert len(answers) == 12 and len({item["question_id"] for item in answers}) == 12
    ...
    print("validated 4 counterbalanced benchmark runs / 48 answers")
