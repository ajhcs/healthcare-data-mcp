# Perimeter Registry: Jefferson handoff

## Boundary

This workspace owns a read-only, deterministic registry: system concept, legal/reporting entities and identifiers, typed effective-dated relationships, evidence observations, four document-specific reporting perimeters, four representative facilities, explicit gaps, and request resolution. The only fixture is Jefferson. `Jefferson` and `Jefferson Health` remain brand concepts, never inferred EINs.
