...
class GitHubMetadataCollector:
    ...
    def _logical_request(
        ...
        *,
        ...
        request_id = f"p{pass_number:01d}-{self._request_counter:05d}-{surface}"
    ...
    def _rest_list(
        ...
        *,
        ...
        while url:
            ...
            visited.add(url)
            page_count += 1
            response = self._logical_request(surface, pass_number, "GET", url)
    ...
    def _download(
        self, surface: str, pass_number: int, url: str, *, allow_unavailable: bool = False
    ) -> tuple[str | None, int, bytes]:
        accepted = {200, 404} if allow_unavailable else {200}
        response = self._logical_request(surface, pass_number, "GET", url, kind="download", accepted_statuses=accepted)
        if response.status_code == 404:
            return None, 0, response.content
        return _sha256(response.content), len(response.content), response.content

    def _collect_discussions(self, pass_number: int) -> list[dict[str, Any]]:
        ...
        records = [_record(item, prefix="category:") for item in categories]
        discussions_query = """
        query($owner:String!,$name:String!,$cursor:String){repository(owner:$owner,name:$name){
        ...
        records.extend(_record(item, prefix="discussion:") for item in discussions)
        comments_query = """
        ...
        for discussion in discussions:
            ...
            records.extend(_record(item, prefix="comment:") for item in comments)
            for comment in comments:
                ...
                records.extend(_record(item, prefix="reply:") for item in replies)
        return records
    ...
    def _collect_pass(self, pass_number: int) -> tuple[dict[str, list[dict[str, Any]]], dict[str, str]]:
        ...
        for surface, (url, key) in simple_lists.items():
            ...
            records[surface] = [_record(item) for item in raw_items[surface]]
        ...
        if raw_items["forks"]:
            raise CollectionError("public forks require recursive repository-scoped metadata capture")

        records["discussions"] = self._collect_discussions(pass_number)
        if repo.get("has_discussions") is False:
            if records["discussions"]:
                raise CollectionError("disabled GitHub discussions returned records")
            states["discussions"] = "disabled"
        ...
        for surface in REQUIRED_SURFACES:
            if not self._surface_evidence[pass_number][surface]:
