...
- The public tool `get_live_gateway_audit_events()` returns recent non-secret audit events for local operations review. Set `MCP_LIVE_GATEWAY_AUDIT_LOG_PATH` to append the same non-secret events to a JSONL file for operational retention. Audit events intentionally omit request payload values, source metadata payloads, evidence payloads, and secrets. Each spec-backed event includes a non-secret `trace_id` and `audit_evidence` object with requested tool scopes, source-claim validation status, provenance status, blocked/degraded reasons, and the same trace ID. When provenance validation blocks a response, the audit event may include `invalid_evidence_paths` with receipt paths and validation errors.
